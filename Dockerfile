...
ADD shim-unsigned-x64-15.6-1.el8.src.rpm /
...
RUN ls -l /*.efi ./usr/share/shim/15*.el8/*/shim*.efi
RUN objcopy -O binary --only-section=.sbat \
    ./usr/share/shim/15*.el8/x64/shimx64.efi /dev/stdout
RUN hexdump -Cv ./usr/share/shim/15*.el8/x64/shimx64.efi > built-x64.hex
RUN hexdump -Cv /shimx64.efi > orig-x64.hex
RUN objdump -h /usr/share/shim/15*.el8/x64/shimx64.efi
RUN diff -u orig-x64.hex built-x64.hex
RUN pesign -h -P -i /usr/share/shim/15*.el8/x64/shimx64.efi
RUN pesign -h -P -i /shimx64.efi
RUN sha256sum /usr/share/shim/15*.el8/x64/shimx64.efi /shimx64.efi 
